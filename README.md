...
## 本 Fork 改动（macOS / 性能 / CPA）

相对上游 Windows 版，本仓库额外提供：

- **macOS Apple Silicon 安装包**（DMG）
- **CPA 等自定义中转**更易设为默认，并支持 **仅显示 CPA 模型**
- **共享 agent 进程**减少新对话冷启动
- **多 Agent 协作可视化**（侧栏树 + 对话卡片 + 浮条）
- **Codex 向视觉**微调

详细说明见 **[docs/FORK-NOTES.md](./docs/FORK-NOTES.md)**（改了哪里、改进了什么、已知缺点）。
